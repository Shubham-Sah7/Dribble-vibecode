...
function SectionHeading({ title }: { title: string }) {
  return (
    <h3 className="text-[10px] font-semibold text-neutral-400 uppercase tracking-widest mb-3.5">
      {title}
    </h3>
  )
}
          ...
          {trendingBuilders.map((builder, i) => (
            <div key={builder.username} className="flex items-center gap-3 group cursor-pointer">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[12px] font-medium text-neutral-700 truncate group-hover:text-neutral-900 transition-colors">
                  {builder.name}
                </div>
        ...
        <div className="space-y-2.5">
          {topTools.map((tool, i) => (
            <div key={tool.name} className="flex items-center gap-3">
              <span className="text-[12px] text-neutral-700 w-14 shrink-0 font-medium">{tool.name}</span>
          ...
          {topByLikes.map((project, i) => (
            <div key={project.id} className="flex items-center gap-2.5 group cursor-pointer">
              ...
              <div
                ...
                style={{ backgroundColor: project.accentColor, opacity: 0.85 }}
              >
                {project.title[0]}
              </div>
              <span className="text-[12px] text-neutral-600 flex-1 truncate group-hover:text-neutral-900 transition-colors">
                {project.title}
              </span>
              <span className="text-[11px] text-neutral-400 tabular-nums shrink-0">
      ...
      {featured && (
        <>
          ...
          <div>
            ...
            <div className="border border-neutral-100 rounded-xl overflow-hidden hover:border-neutral-200 transition-colors cursor-pointer card-lift">
              <div className="h-20 bg-neutral-50 dot-grid relative">
                <div className="absolute inset-0 opacity-[0.07]" style={{ backgroundColor: featured.accentColor }} />
              </div>
              <div className="p-3 border-t border-neutral-100">
                <div className="text-[12px] font-semibold text-neutral-900 mb-1">{featured.title}</div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button className="text-[10px] font-medium text-neutral-500 hover:text-neutral-900 transition-colors">
                    View →
                  </button>
                </div>
              </div>
